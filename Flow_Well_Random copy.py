    ...
    def get_randomized_sets_leave_one_out(self, csv_list, classes_to_include):
        ...
        for entry in csv_list:
            ...
            if len(classes_to_include)==0 or class_for_row in classes_to_include :
                if class_for_row not in val_train_dict:
                    val_train_dict[class_for_row] = {}
                if divide_by not in val_train_dict[class_for_row]:
                    val_train_dict[class_for_row][divide_by] = {}
                if well not in val_train_dict[class_for_row][divide_by]:
                    val_train_dict[class_for_row][divide_by][well] = []
                val_train_dict[class_for_row][divide_by][well].append(entry)

       
        new_training_rows, new_validation_rows = self.get_training_validation_rows(val_train_dict)
